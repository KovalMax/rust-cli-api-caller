use crate::client::{create_client, ApiCaller, ApiRequest, ApiResponse, AuthRequest, AuthResponse};
...
impl ApiResult {
    pub fn create(status: u16, succeed: bool, body: String, row: CsvRow) -> Self {
        ApiResult {
            status,
            succeed,
            body,
...
impl ApiService {
    pub async fn authenticate(&self, request: AuthRequest) -> AuthResponse {
        ...
        while retry_once > 0 {
            let auth_response = self.client.auth_call(request.clone()).await;
    ...
    pub async fn send_api_call(
        ...
        let mut replaced_url = url.replace("{id}", csv_row.id.as_str());
        replaced_url = replaced_url.replace("{market}", csv_row.market.as_str());
        let body_json = serde_json::to_string(&csv_row).unwrap();

        let request = ApiRequest::create(replaced_url, body_json, token, method);

        let api_response = self.client.api_call(request).await;

        self.unwrap_response(api_response, csv_row).await
    }
